...
import { useBackendNotifications } from './useBackendNotifications.js';
...
export function useNotificationMenuState({ truckQueues }) {
  ...
  const visibleNotifications = useBackendNotifications();
  ...
  const { toastNotifications, dismissToast, dismissAllToasts } = useLatestNotificationToast(visibleNotifications);
  const truckQueueMap = useMemo(
    () => new Map((truckQueues ?? []).map((queue) => [queue.packListNo, queue])),
    [truckQueues],
  );
