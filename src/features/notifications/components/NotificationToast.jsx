...
import { getNotificationStyle } from '../utils/notificationAppearance.js';
...
export function NotificationToast({ notifications = [], onClose, onCloseAll }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-2xl shadow-slate-900/25 ring-1 ring-slate-900/8">
      ...
      {/* Scrollable list */}
      <div className="queue-scrollbar max-h-[min(32rem,calc(100vh-8rem))] divide-y divide-slate-200 overflow-y-auto">
        {reversed.map((notification) => {
          ...
          return (
            <div
              key={notification.id}
              className={`notification-toast border-l-4 ${style.leftBorder} ${style.rowBg}`}
            >
              <div className="flex items-start gap-3 px-5 py-4">
                <div className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${style.iconBgClass}`}>
                  <TypeIcon className={`h-5 w-5 ${style.iconClass}`} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`text-base font-bold ${style.titleClass}`}>
                    {notification.title}
                  </p>
