...
import { getNotificationStyle as getNotificationSeverityStyle } from '../utils/notificationAppearance.js';
            ...
            <div className="max-h-96 overflow-y-auto queue-scrollbar">
              {hasNotifications ? (
                visibleNotifications.map((notification) => {
                  const style = getNotificationSeverityStyle(notification.type);
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className={`block w-full border-b border-gray-100 px-5 py-4 text-left transition-colors last:border-b-0 ${style.hoverClass}`}
                      onClick={() => handleNotificationClick(notification)}
                    >
                      <div className="flex items-start gap-3">
                        <div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${style.iconBgClass}`}>
                          <TypeIcon className={`h-4 w-4 ${style.iconClass}`} />
                        </div>
                        <div>
                          <p className={`text-base font-bold ${style.titleClass}`}>{notification.title}</p>
                          <p className="mt-1 text-sm leading-6 text-gray-600">{notification.description}</p>
                          <p className={`mt-2 text-xs font-medium ${style.timeClass}`}>{notification.time}</p>
                        </div>
                      </div>
                    </button>
                  );
