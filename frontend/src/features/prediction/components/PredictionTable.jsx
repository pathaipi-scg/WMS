...
export function PredictionTable({ tableScrollRef, loading, paginatedTrucks, filteredTrucks, trucks, currentPage }) {
  return (
    <div ref={tableScrollRef} className="flex-1 min-h-0 overflow-auto queue-scrollbar">
      <table className="table-fixed w-full min-w-200 text-left text-md">
        <colgroup>
          <col className="w-12" />
          <col className="w-22" />
          <col className="w-20" />
        ...
        <thead className="sticky top-0 z-10 bg-gray-50 text-gray-500 font-medium border-b border-gray-100">
          <tr>
            ...
            <th className="p-3 text-center whitespace-nowrap">ทะเบียน</th>
            <th className="p-3 text-center whitespace-nowrap">ประเภทรถ</th>
            <th className="p-3 text-center whitespace-nowrap">ประเภทคิว</th>
            <th className="p-3 text-center whitespace-nowrap">เวลาเข้า</th>
            <th className="p-3 text-center whitespace-nowrap">คาดว่าเสร็จ</th>
            <th className="p-3 text-center whitespace-nowrap">เวลาเสร็จจริง</th>
            <th className="p-3 text-center whitespace-nowrap">ความคลาดเคลื่อน</th>
          ...
          {loading ? (
            ...
            paginatedTrucks.map((truck, idx) => (
              <tr key={idx} className="border-b border-gray-100 transition-colors hover:bg-gray-50">
                ...
                <td className="p-3 text-center">
                  {truck.isCompleted ? (
                    <span className="inline-flex items-center gap-1 rounded-xl bg-green-100 px-2 py-2 text-xs font-semibold text-green-700">
                      <CheckCircle2 className="h-3 w-3" /> เสร็จแล้ว
                    </span>
                  ) : (
                    <span className="inline-block rounded-xl bg-blue-100 px-2 py-2 text-xs font-semibold text-blue-700">
                      กำลังดำเนินการ
                    </span>
                  )}
                </td>
