import { QUEUE_STATUSES } from '../../constants/queue.js';
import { normalizeQueueType, normalizeStatus } from '../queue/queueTransforms.js';
import {
  getNormalizedText,
  getText,
...
export function resolveMatchedTruckQueue(channel, candidates = []) {
  ...
  const loadingCandidates = matchedCandidates.filter((queue) => {
    const normalizedStatus = getNormalizedText(queue.status);
    return normalizedStatus === QUEUE_STATUSES.loading || normalizedStatus === 'loading';
  });

  if (loadingCandidates.length === 1) {
    return loadingCandidates[0];
  }

  if (loadingCandidates.length > 0) {
    matchedCandidates = loadingCandidates;
  }

  const positionedCandidates = matchedCandidates.filter((queue) =>
    normalizePostLocationKey(queue.post_location_name),
  );
  ...
  return positionedCandidates[0] ?? matchedCandidates[0] ?? null;
}
